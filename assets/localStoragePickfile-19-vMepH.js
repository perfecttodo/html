async function d(){const e=document.createElement("div");e.id="overlay";const t=document.createElement("iframe");t.id="pickerFrame",e.appendChild(t),document.body.appendChild(e),e.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
    ...
    `,t.style.cssText=`
        ...
        border-radius: 8px;
    `,e.offsetWidth,t.offsetWidth,e.style.opacity="1",t.style.opacity="1",t.style.transform="translate(-50%, -50%) scale(1)";const s=Date.now().toString(36)+Math.random().toString(36).substr(2);e.addEventListener("click",i=>{i.target===e&&t.contentWindow.postMessage({type:"cancel",instanceId:s,error:"Clicked outside"},"*")});const o=t.contentDocument||t.contentWindow.document;return o.open(),o.write(`
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8">
    ...
    `),o.close(),new Promise((i,r)=>{const a=async n=>{if(n.data.instanceId!==s)return;const l=()=>{window.removeEventListener("message",a),e.style.opacity="0",setTimeout(()=>{e.parentNode&&e.parentNode.removeChild(e)},300)};if(n.data.type==="select"){l();try{i({filename:n.data.value})}catch(c){r(c)}}else n.data.type==="cancel"&&(l(),r(new Error(n.data.error)))};window.addEventListener("message",a)})}export{d as p};
