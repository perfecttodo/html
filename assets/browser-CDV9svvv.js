import{j as p}from"./index-CGqGl1e2.js";async function f(){const e=document.createElement("div");e.id="overlay";const t=document.createElement("iframe");t.id="pickerFrame",e.appendChild(t),document.body.appendChild(e),e.style.cssText=`
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
    ...
    `),n.close(),new Promise((o,s)=>{const i=async r=>{if(r.data.instanceId!==a)return;const c=()=>{window.removeEventListener("message",i),e.style.opacity="0",setTimeout(()=>{e.parentNode&&e.parentNode.removeChild(e)},300)};if(r.data.type==="select"){c();try{o({filename:r.data.value})}catch(l){s(l)}}else r.data.type==="cancel"&&(c(),s(new Error(r.data.error)))};window.addEventListener("message",i)})}const m="B";function d(e,t){return new Promise((a,n)=>{const o=indexedDB.open(e,1);o.onerror=s=>{n(s.target.error)},o.onsuccess=s=>{a(s.target.result)},o.onupgradeneeded=s=>{const i=s.target.result;i.objectStoreNames.contains(t)||i.createObjectStore(t,{keyPath:"id"})}})}const b=async()=>{const e=await f();return{mode:m,file:{...e,id:e.filename}}};async function g(e){if(!e||typeof e!="object")throw new Error("Invalid or missing path");const t=await d("myDatabase","files");return new Promise((a,n)=>{const i=t.transaction(["files"],"readonly").objectStore("files").get(e.id);i.onerror=r=>{n({code:404,error:`No data found with filename ${e.id}`})},i.onsuccess=r=>{const c=r.target.result;if(!c)n({code:404,error:`No data found with filename ${e.id}`});else{const l=p(c.data);a({content:l,path:{...e,fileName:e.id}})}}})}async function y(e,t){if(!e||typeof e!="object")throw new Error("Invalid or missing data object");if(!t||typeof t!="object")throw new Error("Invalid or missing path");const a=await d("myDatabase","files");return new Promise((n,o)=>{const i=a.transaction(["files"],"readwrite").objectStore("files"),r=JSON.stringify(e,null,2),c=i.put({id:t.fileName,data:r});c.onerror=l=>{o(l.target.error)},c.onsuccess=()=>{n({...t,id:t.fileName})}})}const h="Browser";export{b as pickFile,g as readJsonAttachment,h as type,y as writeObjectToJsonAttachment};
